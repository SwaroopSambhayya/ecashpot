...
import { Alert,Keyboard, AppRegistry,StyleSheet, View,Text,Image,ImageBackground,TextInput,ScrollView,Linking,YellowBox,AsyncStorage } from 'react-native';
...
export default class SignupScreen extends Component{
  ...
  validate=() => {
      ...
      fetch('http://35.234.150.23:8087/api/userregistrationservice/user/', {
        ...
        body: JSON.stringify({"cellnumber":userphone,"email":mail,"password":pass,userProfile:null})
      })
      .then((response) => response.json())
      .then((responseData) => {
        this.saveItem('id_token', responseData.refreshToken),
  ...
  render(){
    const { navigate } = this.props.navigation; 
      return(
        <LinearGradient colors={['#4776e6','#8e54e9']} style={{flex:1,justifyContent:'space-around'}}> 
        <ScrollView  >  
        ...
        <Text>{'\n'}</Text>
        <Item rounded>
          <Input placeholder='Password' placeholderTextColor='white'autoCapitalize='none' secureTextEntry={true} onChangeText={pass=>this.setState({pass})} style={{color:'white', fontWeight:'bold'}} selectionColor={'white'} />
         <Icon type="FontAwesome" name="lock" style={{color:'#fff'}}/>
          </Item>
        <Text>{'\n'}</Text>
        <Item rounded>
          <Input placeholder=' Confirm password' placeholderTextColor='white'autoCapitalize='none' secureTextEntry={true} onChangeText={cpass=>this.setState({cpass})} style={{color:'white',fontWeight:'bold'}} selectionColor={'white'} />
         <Icon type="FontAwesome" name="lock" style={{color:'#fff'}}/>
          </Item> 
           <Text>{'\n'}</Text>
          <View >
